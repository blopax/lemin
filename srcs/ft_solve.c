...
/*   Updated: 2018/04/10 15:56:28 by nvergnac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_build_path(t_info *info, t_path *path_tmp, t_link *link_tmp)
{
	while (link_tmp && path_tmp->flag == FLAG)
	{
		...
		{
			ft_last(PATH)->next = ft_path_init(info, path_tmp,
					ft_get_index(info, link_tmp->name));
			ft_set_black_room(info, ft_get_index(info, link_tmp->name),
					ft_last(PATH));
